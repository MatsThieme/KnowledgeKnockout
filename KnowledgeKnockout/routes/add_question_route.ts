...
export function add_question_route_post(req: Request, res: Response) {
	...
	let topicId: number = parseInt(req.body.topicId);
	let topicBlockId: number = parseInt(req.body.topicBlockId);
	let lastQuestionId: number = 0;

	let sql: string = 'insert into question (content, blockId, topicId) values (?, ?, ?)';
	let inserts: string[] = [questionContent, topicBlockId.toString(), topicId.toString()];
